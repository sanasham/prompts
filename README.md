...
export class UploadService {
  ...
  public async insertToStaging(
    ...
    try {
      ...
      logger.info(`Successfully inserted ${products.length} records to staging for batch ${batchId}`);

      return batchId;
    } catch (error) {
      await transaction.rollback();
      logger.error(`Staging insert failed for batch ${batchId}:`, error);
      throw new AppError(`Failed to insert to staging: ${(error as Error).message}`, 500);
    }
  }
